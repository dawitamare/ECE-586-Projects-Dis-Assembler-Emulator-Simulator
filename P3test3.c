...
// BRANCHES: // Stall for 2 cycles in EX stage
				...
				case 130:	// Branch To Instruction Addr pointed top be Immediate value (Operand1) If Second Reg Contents is Equal to Contents of Third Reg
							/*
								There should be no action here because memory access is not required
								ONLY a Stall
							*/
							break;
	...
	// BRANCHES: // Stall for 2 cycles in EX stage
				...
				case 130:	// Branch To Instruction Addr pointed top be Immediate value (Operand1) If Second Reg Contents is Equal to Contents of Third Reg
							pr[1].RHS = Reg[pr[0].Source2];
							pr[1].LHS = Reg[pr[0].Source1];
							pr[1].Dest = pr[0].Dest; // Dest is PC so no need to add another variable???
							/*if(LHS == RHS)
							{
								PC = Oprnd1;
							}*/
